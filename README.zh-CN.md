...
- 验证证书的 `/ws/relay` 健康过滤；
- 带 generation、digest 与同步 subsystem ack 的 schema v3 last-known-good 激活；
- 对原生 TCP、Secure TCP 和 WSS 上的 `PunchHoleRequest` 与直接
  `RequestRelay` 统一执行可选的严格 Ed25519 连接 JWT 审计/强制认证，并明确保持 UDP
  发起 unsupported；
- 不可变 Relay snapshot、无副作用分配模拟，以及由 mTLS、细粒度 service JWT 和原子
  配置事务保护的独立 Linux Control Agent。
...
| Control Agent | 可选 Starry 组件；v1.2 仅支持 Linux | HBBS 本地控制保持 loopback；Agent 只在私有管理路径提供 mTLS 与细粒度 service JWT 接口，配置写入默认关闭。 |
| 账户/API 服务 | 不包含 | 账户登录、地址簿、设备数据和管理功能需要部署者自行选择独立 API 实现。 |

账户 API 登录、HBBS 信令连接、可选 Starry Control API 与 HBBR 数据转发是相互独立的
协议层。Starry 不会把第三方 API 变成 Relay，不会 fork HBBR，也不会替代 RustDesk 客户端。
...
patch-v1.2.0 的正式发布矩阵为 Docker `linux/amd64`、Linux x86_64 二进制和 amd64 DEB。
ARM 仅尽力保持源码兼容，Windows 为非阻断实验构建；两者都不是 v1.2.0 承诺制品。
...
| [连接认证](https://github.com/q1ngyang/rustdesk-server-starry/wiki/ZH-CN-Connection-Authentication) | JWT profile、audit→enforce 灰度、传输覆盖、故障处理与回滚。 |
| [Control Agent](https://github.com/q1ngyang/rustdesk-server-starry/wiki/ZH-CN-Control-Agent) | Linux 部署、mTLS/service JWT 授权、只读模式、配置事务、恢复与 API 契约。 |
