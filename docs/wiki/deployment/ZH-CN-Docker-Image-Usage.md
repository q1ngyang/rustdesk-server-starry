...
ARM 目前仅作为尽力的源码兼容目标；patch-v1.3.0 不承诺或发布 `linux/arm64` 镜像。

镜像包含修改后的 `hbbs`、保留上游转发并增加公开探测与认证遥测的 `hbbr`，以及未修改的
`rustdesk-utils`。项目部署示例让两个服务使用同一个固定版本的 Starry 镜像，避免 HBBS 与
HBBR 因分别更新而出现版本不一致。
